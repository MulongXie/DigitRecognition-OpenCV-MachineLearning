...
void divide(Mat& img, Mat& org, vector<vector<Point>> contour, int i) {
	...
	if (rec.area() < (org.rows*org.cols) / 400.0 || rec.area() > (org.rows*org.cols) / 10.0) return;   //面积不达标则不放入检测 *剪枝1

	float ratio = 1.0 * rec.size().height / rec.size().width;    //宽高比
	if (ratio < 1.1 || ratio > 3)return;	//宽高比不达标则不放入检测 *剪枝2
	...
	if (fillrate < 0.2 || fillrate>0.95)return;       //区域填充大小不达标则不放入检测（可剪去内轮廓） *剪枝3

	if (rec.tl().x < 5 || rec.tl().y < 5 || rec.br().x > org.cols - 5 || rec.br().y > org.rows - 5) return;    //区域过于接近图像边界则不放入检测 *剪枝4
	...
	if ((float)mycount1 / (70 * 96)<0.2 || (float)mycount2 / (70 * 96)<0.15) return;	   //区域黑白两色填充率不达标不放入检测 *剪枝5
	...
	if (avgrad < 9) return;	                           //边缘梯度检测不达标不放入检测 *剪枝6

	double a = contourArea(contour[i]);
	float rate = a / rec.area();
	bool r;
	if (rate > 0.8)r = 1;           //轮廓面积占比	*剪枝7
	else r = 0;
...
int main() {
	...
	//VideoCapture cap(1);
	VideoCapture cap("F:\\测试视频\\11.5.16.1.5.avi");
